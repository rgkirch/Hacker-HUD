...
os_timer_t myTimer;
SoftwareSerial vfdSerial(D5, D6);
...
float readTemp(Adafruit_MCP9808 &tempsensor) {
  tempsensor.shutdown_wake(
      0); // Don't remove this line! required before reading temp
  ...
  tempsensor.shutdown_wake(
      1); // shutdown MSP9808 - power consumption ~0.1 mikro Ampere
  return f;
...
Option<string> getSiteData(Site site) {
  ...
  Option<string> o(data);
  DynamicJsonBuffer jsonBuffer(2000);
  function<Option<string>(std::string)> f(
      [&o, &jsonBuffer, &site](string str) -> Option<string> {
        JsonObject &o = jsonBuffer.parseObject(str.c_str());
        if (not o.success())
          return Option<string>();
...
void updateSite(Site &site) {
  if (site.lastUpdated == 0 or
      millis() - site.lastUpdated > site.updateInterval) {
    LOG("update site");
...
void timerCallback(void *pArg) {
  if (callbackEnabled) {
    ...
    strncpy(
        buffer,
        NTP.getTimeStr(unixTime + (millis() - unixTimeUpdated) / 1000).c_str(),
        20);
    int len = strlen(buffer);
    memset(&buffer[len], ' ', max(0, 20 - len));
    //    snprintf(buffer, 20, "%d:%d:%d", hourFormat12(unixTime),
...
void vfdPrint(std::string str) {
  if (str.size() == 40) {
    vfdSerial.print(str.c_str());
  }
}
void vfdSetup() { vfdSerial.print("\x1B\x40\x0C"); }
void vfdOverwriteMode() { vfdSerial.print("\x1B\x11"); }
void vfdVirticalScrollMode() { vfdSerial.print("\x1B\x12"); }
void vfdHorizontalScrollMode() { vfdSerial.print("\x1B\x13"); }
void vfdClear() { vfdSerial.print("\x0C"); };
void vfdHome() { vfdSerial.print("\x0B"); };
void serialPrintln(std::string str) { Serial.println(str.c_str()); }
auto tee(std::function<void(std::string)> a, std::function<void(std::string)> b)
...
void setup() {
  ...
  //   });
  //   frames.push_back([&]() {
  //     return openWeatherMapHumidity.get().orElse("no data for weather
  //     humidity");
  //   });
  ...
  //    snprintf(endString, stringLength, "%02d-%02d-%02d", year(endTime),
  //    month(endTime), day(endTime)); snprintf(startString, stringLength,
  //    "%02d-%02d-%02d", year(startTime), month(startTime), day(startTime));
  //    const char *urlTemplate =
  //    "http://api.coindesk.com/v1/bpi/historical/close.json?start=%s&end=%s";
...
void loop() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    // myVFD.print("disconnected from wifi");
    WiFi.begin(ssid, password);
    delay(2000);
    // myVFD.print("connected to wifi");
  }
  //   unixTime = NTP.getTime();
  //   unixTimeUpdated = millis();
  // myVFD.print(NTP.getTimeStr(NTP.getTime()).c_str());
  //   Serial.println(NTP.getTime());
  vfdClear();
  vfdHome();
  std::string what = whatToDisplay(NTP.getTime());
  what.resize(40, ' ');
  //   auto print = tee(vfdPrint, serialPrintln);
  vfdPrint(what);
  serialPrintln(what);
